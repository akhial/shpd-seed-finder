...
def main():
    ...
    result = {
        "schema": "shpd-challenge-parity-fixture/v2",
        "level_columns": [
    ...
    for spec in sys.argv[2:]:
        ...
        current = [run["game_version"], run["game_jar_sha256"], run["runtime"]]
        provenance = current if provenance is None else provenance
        assert current == provenance
        result["runs"][label] = summary
    result["game_version"], result["game_jar_sha256"], result["runtime"] = provenance
    if printing:
