...
@MainActor public final class SpriteAtlas {
    ...
    ///
    /// Returns nil only when the bitmap cannot be allocated.
    public func composedSprite(spriteIndex: Int, pointSize: Int) -> CGImage? {
        let key = ImageKey(spriteIndex: spriteIndex, pointSize: pointSize)
        if let cached = imageCache[key] { return cached }
        guard let rendered = render(spriteIndex: spriteIndex, pointSize: pointSize) else { return nil }
        imageCache[key] = rendered
        return rendered
    }

    private func render(spriteIndex: Int, pointSize: Int) -> CGImage? {
        ...
        if let cropped = items.cropping(to: source) {
        ...
        if let icon = SpriteSheet.ringIconIndex(spriteIndex) {
