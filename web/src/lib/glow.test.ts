...
describe('itemGlow', () => {
  ...
  it('glows black for the curses v4.0.0 added, and not at all for plain items', () => {
    expect(itemGlow({ cursed: true, effect: { name: 'Pressurized', kind: 'curse' } })?.color).toBe('#000000')
    expect(itemGlow({ cursed: true, effect: { name: 'Wondrous', kind: 'curse' } })?.color).toBe('#000000')
...
describe('effectGlows', () => {
  it('gives one glow per named effect, in the order the filter names them', () => {
    expect(effectGlows(['Crystal', 'Venomous']).map((glow) => glow.color)).toEqual(['#0088ff', '#4400aa'])
