...
describe('query serialization', () => {
  ...
  it('writes effect sets as a bare name, a catalog-ordered list, or the any_enchantment shorthand', () => {
    const doc = (effect: RequirementState['effect']) => toQueryDocument({ ...defaultQueryState(), requirements: [plain({ effect })] }).requirements[0]
    expect(doc('Blazing')).toEqual({ kind: 'weapon', effect: 'Blazing' })
    expect(doc(['Blazing'])).toEqual({ kind: 'weapon', effect: 'Blazing' })
    // Out-of-order input is written in catalog order.
    expect(doc(['Vampiric', 'Blocking', 'Projecting'])).toEqual({ kind: 'weapon', effect: ['Blocking', 'Projecting', 'Vampiric'] })
    expect(doc([...weaponEnchantments].reverse())).toEqual({ kind: 'weapon', effect: 'any_enchantment' })
    expect(doc('any_enchantment')).toEqual({ kind: 'weapon', effect: 'any_enchantment' })
    expect(doc([])).toEqual({ kind: 'weapon' })
    expect(canonicalEffect(['Blocking', 'Annoying'], 'melee_weapon')).toEqual(['Blocking', 'Annoying'])
  })
  ...
  it('round-trips effect lists, any_enchantment and upgrade sums', () => {
    ...
    expect(() => fromQueryJson('{"requirements":[{"kind":"weapon","effect":7}]}')).toThrowError(/effect/)
  })
  ...
  it('writes alternative groups as one any_of entry and reads them back with fresh group ids', () => {
    const state: QueryState = { ...defaultQueryState(), requirements: [
      ...
      plain({ kind: 'wand' }),
      plain({ kind: 'thrown_weapon', item: 'shuriken', upgrade: { mode: 'exact', value: 2 }, alternativeGroup: 7 }),
    ...
    expect(JSON.parse(toQueryJson(state))).toEqual({ requirements: [
      { any_of: [{ kind: 'weapon', item: 'spear', upgrade: 3 }, { kind: 'thrown_weapon', item: 'shuriken', upgrade: 2 }] },
      { kind: 'wand' },
      { any_of: [{ kind: 'weapon', item: 'sword' }, { kind: 'weapon', item: 'mace' }] },
    ...
    expect(reread.requirements.map((requirement) => [requirement.item, requirement.alternativeGroup])).toEqual([
      ['spear', 1], ['shuriken', 1], [undefined, undefined], ['sword', 2], ['mace', 2], ['dagger', undefined],
    ])
    expect(toQueryJson(reread)).toBe(toQueryJson(state))
    expect(() => fromQueryJson('{"requirements":[{"any_of":[]}]}')).toThrowError(/any_of/)
    expect(() => fromQueryJson('{"requirements":[{"any_of":[{"any_of":[{"item":"sword"}]}]}]}')).toThrowError(/nest/)
  })
})
