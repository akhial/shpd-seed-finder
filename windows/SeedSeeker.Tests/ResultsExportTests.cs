    ...
    public void AlternativesEffectSetsAndSumsRoundTripThroughTheEngine()
    {
        var query = new QuerySettings
        {
            Requirements = new ObservableCollection<ItemRequirement>([
                ...
                new() { Kind = ItemKind.Ring, Item = ItemCatalog.Find("ring_might"), IdentityGroup = 1, UpgradeSum = new(2, 4) },
                new() { Kind = ItemKind.Ring, Item = ItemCatalog.Find("ring_might"), IdentityGroup = 1, UpgradeSum = new(2, 4), MaximumDepth = 4 },
            ]),
        };
        const string expected = """{"requirements":[{"any_of":[{"kind":"melee_weapon","item":"spear","upgrade":3},{"kind":"thrown_weapon","item":"shuriken","upgrade":2,"effect":["Blocking","Projecting"]}]},{"kind":"armor","effect":"any_enchantment","uncursed":true},{"kind":"ring","item":"ring_might","identity_group":1,"upgrade_sum":{"group":2,"at_least":4}},{"kind":"ring","item":"ring_might","identity_group":1,"max_depth":4,"upgrade_sum":{"group":2,"at_least":4}}]}""";
        var document = ResultsExport.EncodeQueryDocument(query);
        ...
        Assert.Equal([1, 1, null, null, null], imported.Requirements.Select(r => r.AlternativeGroup));
        ...
        Assert.Equal(new UpgradeSum(2, 4), imported.Requirements[3].UpgradeSum);
        Assert.Equal(new UpgradeSum(2, 4), imported.Requirements[4].UpgradeSum);
